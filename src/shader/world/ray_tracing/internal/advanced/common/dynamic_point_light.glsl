...
#endif
// The flicker curve + its speed/intensity config sliders live in the shared helper (also used by
// placed-block-light flicker).
...
vec3 sampleSurfaceDynamicPointLights(SampledSurface surface, vec3 viewDir) {
    ...
    // Flame flicker: a subtle mean~1 multiplier so a carried torch/lantern wavers like a real flame
    // instead of a flat glow (shared curve, full amp for the carried source; gated by the
    // handheld_light_flicker config toggle). Same for every handheld light this frame -> computed once.
    float flicker = (MCVR_HANDHELD_LIGHT_FLICKER != 0)
        ? lightFlickerFactor(worldUBO.flickerTime, 1.0) : 1.0;
